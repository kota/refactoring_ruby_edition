class MountainBike
  ...
  def set_state_from_hash(hash)
    ...
    if hash.hash_key?(:front_suspension_price)
      @front_suspension_price = hash[:front_suspension_price]
    end
    if hash.hash_key?(:rear_suspension_price)
      @rear_suspension_price = hash[:rear_suspension_price]
    end
    if hash.hash_key?(:commission)
      @commission = hash[:commission]
    end
    if hash.hash_key?(:tire_width)
      @tire_width = hash[:tire_width]
    end
    if hash.hash_key?(:front_fork_travel)
      @front_fork_travel = hash[:front_fork_travel]
    end
    if hash.hash_key?(:rear_fork_travel)
